...
int main()
{
	...
	if( true ) {
		ML ml;		//	機械学習オブジェクト
		Bitboard black, white;
		const int  ITR = 100;		//	1ラウンド繰り返し回数
		const int N = 10000;		//	ラウンド数
		const int TOTAL = ITR * N;
		for(int i = 0; i != TOTAL; ++i) {
			init(black, white);		//	盤面初期化
			while( !put_randomly(black, white, 24) ) {	//	24 for 8個空き
				init(black, white);						//	うまくいかなかった場合
			}
			//print(black, white);
			int alpha = 0;
			auto pos = negaAlpha(black, white, alpha);	//	完全読みにより双方最善の場合の石差計算
			ml.learn(black, white, alpha);		//	評価関数パラメータ学習
			if( (i % N) == N - 1 ) {
				//	自乗誤差平均の平方根表示
				cout << (i/N+1) << ": sqrt(err2/N) = " << sqrt(ml.get_err2()/N) << "\n";
				ml.clear_round_err2();		//	自乗誤差累計クリア
			}
		}
		cout << "\n";
		//	学習結果評価用データ出力
	...
	if( false ) {	//	for 負けオセロ
		...
		for(int i = 0; i != TOTAL; ++i) {
			...
			int alpha = 0;
			auto pos = negaAlphaLessWin(black, white, alpha);
			ml.learn(black, white, alpha);		//	評価関数パラメータ学習
			if( (i % N) == N - 1 ) {
				cout << (i/N+1) << ": sqrt(err2/N) = " << sqrt(ml.get_err2()/N) << "\n";
				ml.clear_round_err2();
